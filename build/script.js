...
function createTiles(tilesNumber) {
    ...
    for (let i = 0; i < tilesNumber / 2; i++) {
        currentId++;
        addTile();
        tiles.push(Object.assign({}, allTiles[i]));
        tiles.push(Object.assign({}, allTiles[i]));
    }
    shuffleTiles(tiles);
    currentId = 0;
...
gameZone.addEventListener("click", (e) => {
    ...
    if (target.matches(".back-tile") && !pauseGame) {
        ...
        const tileId = Number(target.closest(".tile-container").dataset.tileId);
...
function updateShortestTime() {
    const highestScore = localStorage.getItem("highestScore") === null
        ? null
    ...
    if (highestScore === null || didBreakHighestScore) {
        ...
        localStorage.setItem("highestScore-name", localStorage.getItem("player's- name"));
        document.querySelector(".saveTime").innerHTML = `
       ${playerName.value} סיימ/ה את המשחק מהר יותר:  ${secondsToFormattedTime(timeCounter)}
    `;
        return;
    }
    document.querySelector(".saveTime").innerHTML = `
...
function checkEndGame() {
    ...
    playSoundEndGame();
    document.querySelector(".timer").textContent = `הזמן שעבר מתחילת המשחק: ${secondsToFormattedTime(timeCounter)}`;
    updateShortestTime();
